...
- Read this file first; `CLAUDE.md`/`GEMINI.md` redirect here and add no durable operating rules.
- Line limits: `AGENTS.md`/`CLAUDE.md`/`GEMINI.md` <=100 lines; other markdown/RAG <=200. If a file would exceed its limit, keep it as an index and move detail into a same-named subdirectory or `docs/spec/**`.
- Context routing: this is the only always-loaded repo instruction file. Do not preload SOP/reference folders; identify the concrete trigger before reading referenced files; read only the smallest matched file/section. If none match, continue from this file only.

## Load Triggers

- `docs/spec/safety-scope.md`: read when an interactive human prompt exceeds 150 words; asks for structural, architectural, debug, root-cause, or similar work; may require pausing, clarification, consent, refusal, or escalation due to ambiguity, safety, privacy, credentials, side effects, identity, authorization, or irreversible actions; or before secrets, sub-agents, or agent injection.
- `docs/spec/dev-loop/git-collaboration.md`: read before branch/worktree create/name/switch/delete; staging, committing, pushing, pulling, rebasing, merging, resetting, cleaning, pruning, opening/reviewing/merging PRs, or protected/default-branch work. Skip for read-only `status`/`diff`/`log` or file inspection.
...
```
Verify files meet the line limits in the Core Rules section above.
