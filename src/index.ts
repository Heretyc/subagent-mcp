...
const ORCHESTRATION_INSTRUCTIONS =
  "ORCHESTRATION MODE (orchestration-mode tool). WHAT IT IS: a per-project toggle for LONG-HORIZON work — tasks that would fill the context window if run to completion inline. OPERATING MODEL: when ON, act as a workflow orchestrator with delegate-default: decompose work and delegate/offload by default. INLINE BY RIGHT: steps bound to main-session-only capability may stay inline, including MCP tools sub-agents can't inherit, interactive/consent tools, and tight verify loops; state which and why. MUST DELEGATE/OFFLOAD: pure compute and any payload >50KB or >200 lines go through temp scratch-file path handoff (%TEMP% on Windows, /tmp on POSIX); keep the main/orchestrator context lean. CONFLICT ORDER: safety-scope > user instruction this turn > delegate-default. A user tool-pin re-partitions work; it does not suspend mode. PERSISTENCE: enabling writes a per-project marker that PERSISTS across process restarts/sessions until it is disabled with explicit user permission (it does NOT reset on a new session). CARRYOVER: if the mode was already ON at the start of this session (inherited from a prior session), the bundled hook prepends a CARRYOVER notice ONCE per project marker and you MUST notify the user it auto-activated, ask whether to keep it ON, and advise whether keeping it fits the user's request this session. DISABLE GOVERNANCE: never disable on your own initiative — disable ONLY with EXPLICIT user permission, after first explaining WHAT orchestration mode is and WHY you want to disable it, and request that permission with the provider-appropriate interactive tool: AskUserQuestion on Claude, request-user-input on Codex. PROPOSE-OFF: you MAY on your own initiative propose disabling when task fit is wrong (bounded, interactive, or MCP-bound); explain WHAT mode is and WHY it mis-fits, then request permission via AskUserQuestion (Claude) or request-user-input (Codex). Only explicit approval may call orchestration-mode enabled:false; if declined, continue under inline-by-right, ask once per topic, and never re-nag. Per-turn injection fires only in CLI hosts that load the bundled hook; desktop hosts toggle the marker but inject nothing (documented degradation).";
