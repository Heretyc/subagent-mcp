...
`launch_agent` also returns `permissions_applied` (the applied ceiling plus rule and directory counts). When a ceiling cap applies, `permissions_applied.ceiling_cap` carries exactly `reason` and `source_paths`, and never parse text or config contents; an uncapped ceiling omits `ceiling_cap` (see [docs/spec/permissions/ceiling-modes.md](spec/permissions/ceiling-modes.md)).
...
Full `configure` tool reference: [tools/configure.md](tools/configure.md).

Full `swarm` tool reference: [tools/swarm.md](tools/swarm.md).
