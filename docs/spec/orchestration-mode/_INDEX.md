...
- **Server `instructions`** (`ORCHESTRATION_INSTRUCTIONS` in `src/index.ts`,
  passed as the second `McpServer` arg â†’ emitted in the `initialize` result)
...
- **Per-turn directives** (`directives/**`): the FULL `<subagent-mcp state="..." kind="...">`
  ...
  steady state is the per-prompt reminder cadence â€” the LONG mode-specific
  `<subagent-mcp state="..." kind="...">` block (`reminder-on.md` /
  `reminder-off-<provider>.md`) every 5th prompt, and between prompts a
  state-aware short pointer â€” `short-on.md` when ON, `short-off.md` when OFF
...
**Purpose.** Orchestration mode is for LONG-HORIZON work â€” tasks expected to
...
an observable, independently-verifiable artifact â€” it does NOT 1-shot
...
`orchestration-<provider>.md`, `reminder-on.md`, `short-on.md`) â€” it governs how
...
- **The hook** (`dist/hooks/orchestration-claude*.js` /
  ...
  the hook never share memory â€” the marker file on disk is the only channel.
...
lowercases on win32, and strips a trailing `/` â€” so `C:\X\` and `c:/x` key to
...
- A per-prompt counter (`src/orchestration/reminder.ts`, state file
  ...
  prompt between emits the state-aware short pointer â€” `short-on.md` when ON,
...
A user-approved disable is per-session and time-bounded: `src/orchestration/marker.ts`
expires the disable record after the ~2-hour backstop, re-arming ON even if no
new session started; see Â§10 of the directive architecture.
...
- **CARRYOVER** (`owner_session` is a real, *different* session â€” or current is
...
Whole body is fail-safe (any error â†’ `''`). The Codex `SessionStart` path applies
...
provider tool (`AskUserQuestion` for Claude, `request-user-input` for Codex â€”
...
| Claude Desktop | yes | **no** â€” no `UserPromptSubmit` hook host |
| Codex Desktop | yes | **no** â€” no `UserPromptSubmit` hook host |
...
Distinguish these marker-aware-but-hookless desktop hosts from the no-tag
fail-safe: a desktop host still runs the `orchestration-mode` toggle (the marker
flips) and simply injects no `<subagent-mcp state="...">` tag, whereas the
architecture doc's fail-safe covers a session where NO injection tag is present
at all and therefore defaults to ON. The two are not the same condition â€” a
hookless desktop host is a known, expected absence of injection, not the
unknown-state fail-safe â€” see
[orchestration-directive-architecture.md](../dev-loop/orchestration-directive-architecture.md)
Â§5.
## The five locked decisions
1. Per-project temp marker file keyed by a hash of the working directory.
2. (SUPERSEDED 2026-06-11 by the per-prompt reminder counter â€” see Cadence.)
