...
| `configure` | `action` enum (`"list"/"get"/"set"`), `key?` string, `value?` string | list: `{ ok, action, restart_required: false, keys, patterns }`; get: `{ ok, action, key, value, scope, path, settable, restart_required: false, restart_required_on_set, source? }`; set: `{ ok, action, key, value?, status, path, backup, restart_required, message?, source? }`; error: `{ ok: false, action, key?, restart_required: false, error }` with `isError: true`; secret-matching values and all env values are always redacted; global/update/mode keys return `status: "coached"` instead of writing |
