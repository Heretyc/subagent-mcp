...
plugin install root). Running both paths in one workspace duplicates the server
and causes a **double** hook injection (plugin hook + settings.json hook both
fire). On a machine using this standalone install:
