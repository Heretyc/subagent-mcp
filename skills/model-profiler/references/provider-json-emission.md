...
3. **Universe completeness (table-derived)** — the model@effort universe is the union of all
   pairings across both branches; each category in BOTH branches must contain every universe
   pairing exactly once (no duplicates, no omissions). `metadata.model_effort_universe` is NOT
   consulted (it no longer exists on the canonical table).
