...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        print("  extract [--backend B] [--provider P] [--root R] <files>  direct LLM extraction (supports --backend local for keyless harness dispatch)")
    ...
    if cmd == "install":
        ...
        # graphify extract --backend local [--provider claude|codex] [--root .] file [file ...]
        # Thin CLI wrapper around llm.extract_files_direct so --backend local reaches extract.
        import argparse as _ap
        ...
        file_paths = [_Path(f) for f in opts.files]
        ...
        result = extract_files_direct(
            ...
            model=opts.model,
            root=_Path(opts.root),
            provider=opts.provider,
        )
        out_json = json.dumps(result, indent=2)
        if opts.out:
            _Path(opts.out).write_text(out_json, encoding="utf-8")
            print(f"[graphify extract] result written to {opts.out}")
        else:
            print(out_json)
