...
_MARKDOWN_INCLUDE_PATH = ".graphify/markdown-include.txt"
...
def classify_file(path: Path) -> FileType | None:
    ...
    if ext in MD_EXTENSIONS:
        # Markdown has a deterministic structural extractor; keep the LLM
        # document path as a fallback for other doc formats.
        return FileType.CODE
...
def _load_markdown_include(root: Path) -> list[tuple[Path, str]]:
    """Read markdown structural-ingestion allowlist patterns."""
    return _load_pattern_file(root, _MARKDOWN_INCLUDE_PATH)


def _pattern_matches(path: Path, rel: str, pattern: str) -> bool:
    """Return True if *rel* or path.name matches a Graphify glob pattern."""
    parts = rel.split("/")
    candidates = [pattern]
    if "/**/" in pattern:
        # Treat foo/**/*.md as matching both foo/bar.md and foo/a/bar.md.
...
def _markdown_pattern_matches(rel: str, pattern: str) -> bool:
    """Markdown allowlist matching: repo-relative paths, recursive dirs, globs."""
    ...
    return any(fnmatch.fnmatch(rel, candidate) for candidate in candidates)


def _matches_markdown_patterns(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    if not patterns:
        return False
    for anchor, pattern in patterns:
        try:
            rel = str(path.resolve().relative_to(anchor.resolve())).replace(os.sep, "/")
        except ValueError:
            try:
                rel = str(path.resolve().relative_to(root.resolve())).replace(os.sep, "/")
            except ValueError:
                continue
        if _markdown_pattern_matches(rel, pattern):
            return True
    return False


def _could_contain_markdown_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    if not patterns:
        return False
    rels: list[str] = []
    for base in {root.resolve(), *(anchor.resolve() for anchor, _ in patterns)}:
        try:
            rels.append(str(path.resolve().relative_to(base)).replace(os.sep, "/").strip("/"))
        except ValueError:
            pass
    for rel in rels:
        if not rel:
        ...
        for _, pattern in patterns:
            p = pattern.replace("\\", "/").strip().strip("/")
            if not p:
                continue
            if p == rel or p.startswith(rel + "/") or _markdown_pattern_matches(rel, pattern):
                return True
    return False


def is_markdown_included(path: Path, root: Path, patterns: list[tuple[Path, str]] | None = None) -> bool:
    """Return True if a Markdown path is explicitly allowlisted for ingestion."""
    if path.suffix.lower() not in MD_EXTENSIONS:
        return False
    root = root.resolve()
    loaded = _load_markdown_include(root) if patterns is None else patterns
    return _matches_markdown_patterns(path, root, loaded)


def could_contain_markdown_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]] | None = None) -> bool:
    """Return True if a directory may contain allowlisted Markdown."""
    root = root.resolve()
    loaded = _load_markdown_include(root) if patterns is None else patterns
    return _could_contain_markdown_included_path(path, root, loaded)
...
def detect(root: Path, *, follow_symlinks: bool = False) -> dict:
    ...
    markdown_include_patterns = _load_markdown_include(root)
    ...
    for scan_root in scan_paths:
        ...
        for dirpath, dirnames, filenames in os.walk(scan_root, followlinks=follow_symlinks):
            ...
            if not in_memory_tree:
                ...
                # Hidden dirs remain skipped by default, but .graphifyinclude can
                # opt in the exact hidden subtrees needed to reach allowlisted files.
                dirnames[:] = [
                    d for d in dirnames
                    if (not d.startswith(".")
                        or _could_contain_included_path(dp / d, root, include_patterns)
                        or could_contain_markdown_included_path(dp / d, root, markdown_include_patterns))
    ...
    for p in all_files:
        ...
        if not in_memory:
            ...
            markdown_included = is_markdown_included(p, root, markdown_include_patterns)
            if hidden and not (_is_included(p, root, include_patterns) or markdown_included):
                continue
            if p.suffix.lower() in MD_EXTENSIONS and not markdown_included:
                continue
    ...
    return {
        ...
        "graphifyinclude_patterns": len(include_patterns),
        "markdown_include_patterns": len(markdown_include_patterns),
    }
