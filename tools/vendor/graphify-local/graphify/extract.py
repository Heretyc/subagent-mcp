...
from .extract_markdown import MD_EXTENSIONS, extract_markdown
...
def _extract_generic(path: Path, config: LanguageConfig) -> dict:
    ...
    def walk_calls(node, caller_nid: str) -> None:
        ...
        if node.type in config.call_types:
            ...
            func_node = None  # not set on every language branch (e.g. C#); guard L1207 read
...
def _build_code_symbol_index(nodes: list[dict]) -> dict[str, list[str]]:
    """Build exact simple-name lookup for markdown code-span mentions."""
    matches: dict[str, list[tuple[bool, str]]] = {}
    for node in nodes:
        ...
        if source_name and label == source_name:
            continue
        key = label.strip("()").lstrip(".")
        if key != label and not label.endswith("()"):
            continue
        if key:
            normalized = str(node.get("source_file", "")).replace("\\", "/").lstrip("./")
            excluded = any(part in normalized for part in ("/Library/", "/Packages/", "/PackageCache/", "/tools/vendor/"))
            excluded = excluded or normalized.startswith(("Library/", "Packages/", "PackageCache/", "tools/vendor/"))
            project = not excluded and (
                normalized.startswith(("Assets/", "scripts/", "Scripts/"))
                or any(part in normalized for part in ("/Assets/", "/scripts/", "/Scripts/"))
            )
            matches.setdefault(key, []).append((project, nid))

    index: dict[str, list[str]] = {}
    for key in sorted(matches):
        project_ids = sorted(nid for is_project, nid in matches[key] if is_project)
        if project_ids:
            index[key] = project_ids
    return index
...
def extract(paths: list[Path], cache_root: Path | None = None) -> dict:
    ...
    result_by_path: dict[Path, dict] = {}
    markdown_paths = [path for path in paths if path.suffix.lower() in MD_EXTENSIONS]
    structural_paths = [path for path in paths if path.suffix.lower() not in MD_EXTENSIONS]
    ...
    for result in result_by_path.values():
        code_nodes.extend(result.get("nodes", []))
    code_index = _build_code_symbol_index(code_nodes)

    offset = len(structural_paths)
    for j, path in enumerate(markdown_paths, start=offset):
        if total >= _PROGRESS_INTERVAL and j % _PROGRESS_INTERVAL == 0 and j > 0:
            print(f"  AST extraction: {j}/{total} files ({j * 100 // total}%)", flush=True)
        extractor = _DISPATCH.get(path.suffix.lower())
        if extractor is None:
            continue
        result_by_path[path] = extractor(path, root=cache_root or root, code_index=code_index)

    per_file = [result_by_path[path] for path in paths if path in result_by_path]
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    if target.is_file():
        ...
        if target.suffix.lower() in MD_EXTENSIONS:
            from graphify.detect import is_markdown_included
            return [target] if is_markdown_included(target, ignore_root) else []
        return [target]
    _EXTENSIONS = {
        ".py", ".js", ".ts", ".tsx", ".go", ".rs",
    ...
    from graphify.detect import (
        ...
        _load_markdown_include,
        _is_ignored,
        could_contain_markdown_included_path,
        is_markdown_included,
    )
    ignore_root = root if root is not None else target
    patterns = _load_graphifyignore(ignore_root)
    markdown_patterns = _load_markdown_include(ignore_root)

    def _ignored(p: Path) -> bool:
        return bool(patterns and _is_ignored(p, ignore_root, patterns))

    def _include_file(p: Path) -> bool:
        return p.suffix.lower() not in MD_EXTENSIONS or is_markdown_included(p, ignore_root, markdown_patterns)

    def _hidden_allowed(p: Path) -> bool:
        return p.suffix.lower() in MD_EXTENSIONS and is_markdown_included(p, ignore_root, markdown_patterns)
    ...
    if not follow_symlinks:
        ...
        for ext in sorted(_EXTENSIONS):
            results.extend(
                ...
                if (not any(part.startswith(".") for part in p.parts) or _hidden_allowed(p))
                and _include_file(p)
                and not _ignored(p)
            )
    ...
    for dirpath, dirnames, filenames in os.walk(target, followlinks=True):
        ...
        if any(part.startswith(".") for part in dp.parts):
            if not could_contain_markdown_included_path(dp, ignore_root, markdown_patterns):
                dirnames.clear()
                continue
        for fname in filenames:
            p = dp / fname
            if p.suffix in _EXTENSIONS and (not fname.startswith(".") or _hidden_allowed(p)) and _include_file(p) and not _ignored(p):
                results.append(p)
    return sorted(results)
