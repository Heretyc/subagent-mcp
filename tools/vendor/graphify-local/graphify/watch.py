...
from graphify.detect import CODE_EXTENSIONS, DOC_EXTENSIONS, MD_EXTENSIONS, PAPER_EXTENSIONS, IMAGE_EXTENSIONS, is_markdown_included

_WATCHED_EXTENSIONS = CODE_EXTENSIONS | DOC_EXTENSIONS | PAPER_EXTENSIONS | IMAGE_EXTENSIONS
_CODE_EXTENSIONS = CODE_EXTENSIONS | MD_EXTENSIONS
...
def _rebuild_code(watch_path: Path, *, follow_symlinks: bool = False) -> bool:
    ...
    try:
        ...
        for node in result.get("nodes", []):
            node["_degree"] = degree.get(node.get("id"), 0)
        ...
        if existing_graph.exists():
            try:
                ...
                doc_section_ids = {
                    ...
                    if n.get("type") == "doc_section" or n.get("node_type") == "doc_section"
                }
                sem_nodes = [
                    n for n in existing.get("nodes", [])
                    if n.get("file_type") != "code" and n.get("id") not in doc_section_ids
                ]
                sem_edges = [e for e in existing.get("links", existing.get("edges", []))
                             if e.get("source") not in doc_section_ids
                             and e.get("target") not in doc_section_ids
                             and (e.get("confidence") in ("INFERRED", "AMBIGUOUS")
        ...
        from graphify.semantic_doc_links import run_or_load
        doc_nodes = [
            n for n in result.get("nodes", [])
            if n.get("type") == "doc_section" or n.get("node_type") == "doc_section"
        ]
        semantic_doc_edges = run_or_load(watch_root, result.get("nodes", []), doc_nodes)
        if semantic_doc_edges:
            result["edges"].extend(semantic_doc_edges)
        for node in result.get("nodes", []):
            node.pop("_degree", None)
        ...
        # AST/update rebuilds are authoritative for code plus deterministic
        # Markdown sections; allow node-count drops when files leave scope.
        json_written = to_json(G, communities, str(out / "graph.json"), force=True)
        if not json_written:
            return False
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    def _markdown_allowed(path: Path) -> bool:
        return path.suffix.lower() not in MD_EXTENSIONS or is_markdown_included(path, watch_path)
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            if path.suffix.lower() not in _WATCHED_EXTENSIONS:
                return
            if path.suffix.lower() in MD_EXTENSIONS and not _markdown_allowed(path):
                return
            if any(part.startswith(".") for part in path.parts) and not _markdown_allowed(path):
                return
