...
| MINDSTUDIO-2026 | MindStudio. (2026). *GPT-5.5 vs Claude Opus 4.7 for agentic coding*. https://www.mindstudio.ai/blog/gpt-5-5-vs-claude-opus-4-7-agentic-coding-2 | Agentic loop comparison; wrong-file commit risk; GPT-5.5 literal instruction-following → `failure-modes.md`, `model-profiles.md` |
...
| TAXONOMY-MIGRATION-2026 | Blackburn, L., et al. (2026). *Cross-provider taxonomy migration: 9-spine to 10-spine* [internal adversarial debate — NOT an external APA source]. [INTERNAL PROVENANCE — 3-reviewer 4-round adversarial debate (Claude lens + Codex lens); ratified 2026-06; canonical record kept in the work-categories leaf (see Supports column). Do not cite as external provenance.] | `decision-rationale.md` CR-9; `work-categories.md` 10-spine taxonomy |
| BENCHMARK-SOURCES-KB | (Internal cross-reference — not APA.) Canonical external benchmark source list (leaderboard URLs, eval datasets, source tiers for all 10 categories) lives at `skills/model-profiler/references/benchmark-sources.md`. APA citations for individual primary papers/vendor docs remain in this table under their own source IDs. | `model-profiles.md`; `decision-rationale.md` CR-9; `retrieval-map.md` |
